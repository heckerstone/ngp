...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedNgpAccounts") && NRS.getStrItem("savedNgpAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedNgpAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var ngpAddress = new NgpAddress();
                if (ngpAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedNgpAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedNgpAccounts');
		} else {
			NRS.setStrItem("savedNgpAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedNgpAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedNgpAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedNgpAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedNgpAccounts") && NRS.getStrItem("savedNgpAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedNgpAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Ngp passphrase.";
			html += "</p>";
			...
			html += "you can loose your NGP! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of NGP now or in the future.";
			html += "</p>";
			html += "</div>";
