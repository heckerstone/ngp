...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumNGP",
			addonText: "NGP",
			addonI18n: "ngp_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_ngp', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceNGP';
		context['addonText'] = 'NGP';
		context['addonI18n'] = 'ngp_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_ngp', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
