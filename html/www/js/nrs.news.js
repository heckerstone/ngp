	...
	NRS.pages.news = function() {
      ...
      var settingsReddit = {
         ...
         "filter": function(entry, tokens) {
            return tokens.title.indexOf("Donations toward") == -1 && tokens.title.indexOf("NGP tipping bot has arrived") == -1
         },
         "ssl": ssl
      };

      $("#ngpforum_news").rss("https://ngpforum.org/index.php?type=rss;action=.xml", settings, NRS.newsLoaded);
      $("#reddit_news").rss("http://www.reddit.com/r/NGP/.rss", settingsReddit, NRS.newsLoaded);
      $("#ngpcoin_blogspot_news").rss("http://ngpcoin.blogspot.com/feeds/posts/default", settings, NRS.newsLoaded);
      $("#ngper_news").rss("http://ngper.org/feed/", settings, NRS.newsLoaded);
		NRS.pageLoaded();
	};
