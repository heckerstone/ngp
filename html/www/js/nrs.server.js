...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var ngpFields = [
                ...
                ["priceNGP", "priceNQT"],
                ["refundNGP", "refundNQT"],
                ["discountNGP", "discountNQT"],
                ["phasingQuorumNGP", "phasingQuorum"],
                ["phasingMinBalanceNGP", "phasingMinBalance"],
                ["controlQuorumNGP", "controlQuorum"],
                ["controlMinBalanceNGP", "controlMinBalance"],
                ["controlMaxFeesNGP", "controlMaxFees"],
                ["minBalanceNGP", "minBalance"],
                ["shufflingAmountNGP", "amount"],
                ["monitorAmountNGP", "amount"],
                ["monitorThresholdNGP", "threshold"]
            ];

            for (i = 0; i < ngpFields.length; i++) {
                var ngpField = ngpFields[i][0];
                var nqtField = ngpFields[i][1];
                if (ngpField in data) {
                    data[nqtField] = NRS.convertToNQT(data[ngpField]);
                    delete data[ngpField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToNGP(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (ngpAddress.set(accountId)) {
                accountRS = ngpAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/ngp";
        } else {
