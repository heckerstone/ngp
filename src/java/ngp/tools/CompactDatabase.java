 ...
 *
 *   java -cp "classes:lib/*:conf" ngp.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dngp.runtime.mode=desktop ngp.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize NGP properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "ngp.testDb" : "ngp.db";
        String dbType = NGP.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = NGP.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = NGP.getDbDir(NGP.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = NGP.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = NGP.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = NGP.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "ngp.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "ngp.mv.db");
            if (!dbFile.exists()) {
