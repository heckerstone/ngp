...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws NgpException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = NGP.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws NgpException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws NgpException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : NGP.getBlockchain().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new NgpException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new NgpException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
...
//        return version;
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_NGP));
                    buffer.putInt((int) (feeNQT / Constants.ONE_NGP));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws NgpException.NotValidException {
        try {
            ...
            if (buffer.hasRemaining()) {
                throw new NgpException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (NgpException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws NgpException.NotValidException {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws NgpException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new NgpException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws NgpException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new NgpException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
...
//                || NGP.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws NgpException.ValidationException {
    	// feeNQT <= 0 update
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                ...
                || type == null) {
            throw new NgpException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new NgpException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new NgpException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new NgpException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new NgpException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion(this.version)) {
                throw new NgpException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new NgpException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }
        ...
        if (!validatingAtFinish) {
            ...
            if (feeNQT < minimumFeeNQT) {
                throw new NgpException.NotCurrentlyValidException(String.format("Transaction fee %f NGP less than minimum fee %f NGP at height %d",
                        ((double) feeNQT) / Constants.ONE_NGP, ((double) minimumFeeNQT) / Constants.ONE_NGP, blockchainHeight));
            }
            if (blockchainHeight > Constants.FXT_BLOCK && ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new NgpException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_NGP);
        }
        return totalFee;
    }
