...
import static ngp.http.JSONResponses.INCORRECT_BLOCK;
import static ngp.http.JSONResponses.INCORRECT_HEIGHT;
import static ngp.http.JSONResponses.INCORRECT_TIMESTAMP;
import static ngp.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = NGP.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                blockData = NGP.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = NGP.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = NGP.getBlockchain().getLastBlock();
        }
