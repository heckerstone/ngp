...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (NGP.getBooleanProperty("ngp.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = NGP.getIntProperty("ngp.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = NGP.getStringProperty("ngp.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && NGP.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, NGP.getEpochTime() + blacklistingPeriod);
