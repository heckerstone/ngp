...
import static ngp.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static ngp.http.JSONResponses.INCORRECT_DEADLINE;
import static ngp.http.JSONResponses.INCORRECT_EC_BLOCK;
import static ngp.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static ngp.http.JSONResponses.INCORRECT_WHITELIST;
import static ngp.http.JSONResponses.MISSING_DEADLINE;
import static ngp.http.JSONResponses.MISSING_SECRET_PHRASE;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                NGP.getBlockchain().getHeight() + 1,
                NGP.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws NgpException {
        ...
        if (ecBlockId != 0 && ecBlockId != NGP.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = NGP.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = NGP.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (NgpException.NotYetEncryptedException ignore) {}
        ...
        } catch (NgpException.NotYetEnabledException e) {
