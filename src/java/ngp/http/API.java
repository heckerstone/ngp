...
import static ngp.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static ngp.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static ngp.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = NGP.getStringProperty("ngp.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = NGP.getIntProperty("ngp.maxAPIRecords");
    static final boolean enableAPIUPnP = NGP.getBooleanProperty("ngp.enableAPIUPnP");
    public static final int apiServerIdleTimeout = NGP.getIntProperty("ngp.apiServerIdleTimeout");
    public static final boolean apiServerCORS = NGP.getBooleanProperty("ngp.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(NGP.getStringListProperty("ngp.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = NGP.getStringListProperty("ngp.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = NGP.getStringListProperty("ngp.allowedBotHosts");
        ...
        boolean enableAPIServer = NGP.getBooleanProperty("ngp.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : NGP.getIntProperty("ngp.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : NGP.getIntProperty("ngp.apiServerSSLPort");
            final String host = NGP.getStringProperty("ngp.apiServerHost");
            disableAdminPassword = NGP.getBooleanProperty("ngp.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = NGP.getBooleanProperty("ngp.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(NGP.getUserHomeDir()).resolve(Paths.get(NGP.getStringProperty("ngp.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(NGP.getStringProperty("ngp.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(NGP.getStringProperty("ngp.keyStoreType"));
                List<String> ciphers = NGP.getStringListProperty("ngp.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = NGP.getStringProperty("ngp.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{NGP.getStringProperty("ngp.apiWelcomeFile")});
            }

            String javadocResourceBase = NGP.getStringProperty("ngp.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/ngp");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(NGP.getIntProperty("ngp.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/ngp-proxy");
            ...
            if (!NGP.getBooleanProperty("ngp.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/ngp", "/ngp-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(ngp.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (NGP.getBooleanProperty("ngp.apiFrameOptionsSameOrigin")) {
