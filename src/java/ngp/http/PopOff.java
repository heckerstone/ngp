...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            ...
            if (numBlocks > 0) {
                blocks = NGP.getBlockchainProcessor().popOffTo(NGP.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = NGP.getBlockchainProcessor().popOffTo(height);
            } else {
        ...
        if (keepTransactions) {
            blocks.forEach(block -> NGP.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
