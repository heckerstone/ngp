...
import ngp.TransactionProcessor;
import ngp.NGP;
import ngp.AccountLedger.LedgerEntry;
 ...
 * has not been received within ngp.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by ngp.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = NGP.getIntProperty("ngp.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(NGP.getIntProperty("ngp.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = NGP.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = NGP.getTransactionProcessor();
    ...
    /** NGP event listeners */
    private final List<NgpEventListener> ngpEventListeners = new ArrayList<>();
     ...
     *
     * NGP event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of NGP event registrations
     ...
     *
     * @param   eventRegistrations      NGP event registrations
     * @throws  EventListenerException  Invalid NGP event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<NgpEventListener> it = ngpEventListeners.iterator();
                while (it.hasNext()) {
                    NgpEventListener listener = it.next();
                ...
                if (addListener) {
                    NgpEventListener listener = new NgpEventListener(event);
                    listener.addListener();
                    ngpEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            ngpEventListeners.forEach(NgpEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * NGP event listener
     */
    private class NgpEventListener {

        /** Event handler */
        private final NgpEventHandler eventHandler;

        /**
         * Create the NGP event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public NgpEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the NGP listener for this event
         */
        ...
        /**
         * Remove the NGP listener for this event
         */
        ...
        /**
         * Return the hash code for this NGP event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two NGP events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof NgpEventListener) &&
                    eventHandler.equals(((NgpEventListener)obj).eventHandler));
        }

        /**
         * NGP listener event handler
         */
        private abstract class NgpEventHandler {
            ...
            /**
             * Create the NGP event handler
             *
             * @param   eventRegistration   Event registration
             */
            public NgpEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof NgpEventHandler) &&
                                        owner == ((NgpEventHandler)obj).owner &&
                                        accountId == ((NgpEventHandler)obj).accountId &&
                                        event == ((NgpEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends NgpEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends NgpEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends NgpEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends NgpEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           NGP listener event
        ...
        /**
         * Return the NGP listener event
         *
         * @return                  NGP listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
