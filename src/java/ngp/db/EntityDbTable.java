...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && NGP.getBlockchainProcessor().isScanning() ?
                    Math.max(NGP.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return NGP.getBlockchain().getHeight() <= height && ! (isPersistent() && NGP.getBlockchainProcessor().isScanning());
    }
