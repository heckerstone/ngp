...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws NgpException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new NgpException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountNQT / Constants.ONE_NGP));
                buffer.putInt((int) (totalFeeNQT / Constants.ONE_NGP));
            } else {
...
//            }
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceNGP();
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f NGP to forger at height %d", ((double)backFees[i])/Constants.ONE_NGP, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f NGP at height %d", ((double)totalBackFees)/Constants.ONE_NGP, this.height);
        }
