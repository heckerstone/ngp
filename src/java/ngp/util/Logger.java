...
/**
 * Handle logging for the NGP node server
 */
public final class Logger {
     ...
     * The ngp/conf/logging-default.properties and ngp/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "ngp.util.NgpLogManager");
        if (!(LogManager.getLogManager() instanceof NgpLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("ngp.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                NGP.loadProperties(loggingProperties, "logging-default.properties", true);
                NGP.loadProperties(loggingProperties, "logging.properties", false);
                NGP.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(ngp.NGP.class);
        enableStackTraces = NGP.getBooleanProperty("ngp.enableStackTraces");
        enableLogTraceback = NGP.getBooleanProperty("ngp.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof NgpLogManager) {
            ((NgpLogManager) LogManager.getLogManager()).ngpShutdown();
        }
    }
