...
Unpack the ngp-client.zip package and open a shell in the resulting ngp
...
the ngp_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.ngp for
configuration files, logs, and the ngp_db database. Unlike run.sh, start.sh
...
message "Ngp server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:7877 ,
where the Ngp UI should now be available.
...
to the Ngp installation directory, as the use of special characters may result
...
conf/ngp-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
ngp-default.properties directly, but create a new conf/ngp.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from ngp-default.properties, the
settings in ngp.properties override those in ngp-default.properties. This way,
when upgrading the software, you can safely overwrite ngp-default.properties
with the updated file from the new package, while your customizations remain
safe in the ngp.properties file.


How to contribute?

There are many ways to contribute to Ngp. Here are some examples:
...
The Ngp software is a client-server application. It consists of a java server
...
The user interface is available on port 7877. This port also accepts http API
requests which other Ngp client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
ngp_db directory. When upgrading, you should not delete the old ngp_db
...
ngp_db, except that it will take some extra time to download the blockchain
from scratch.

The default Ngp client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file ngp.log, which gets
overwritten at restart. In case of an error, the ngp.log file may contain
helpful information, so include its contents when submitting a bug report.

In addition to the default user interface at http://localhost:7877 , the
following urls are available:

http://localhost:7877/test - a list of all available http API requests, very
...
http://localhost:7877/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:7877/doc - a javadoc documentation for client developers who
...
ngp.jar file using the enclosed jar.sh script, and then ngp.jar should be
